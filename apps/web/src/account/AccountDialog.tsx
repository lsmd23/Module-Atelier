...
import { api } from "../api";
import type { AccountUser, ProjectMember } from "../api/types";
...
/*
 * 账户管理对话框（右上角头像 →「账户与安全」）。
 * 数据 MOCK ONLY（auth 契约未冻结）；交互与字段为商用账户系统的完整骨架。
 */
    ...
    <div className="text-sm">
        ...
        <span className="flex gap-2">
          ...
          <button
            ...
            onClick={() => {
              setBusy(true);
              void api
                .updateProfile(user.id, { displayName: name.trim() })
                .then((u) => {
                  setUser(u);
                  setSaved(true);
                })
      ...
      <FieldRow label="邮箱">
        <span className="flex items-center gap-2">
          {user.email}
          {user.emailVerified ? (
            <span className="rounded-full bg-forest/15 px-2 py-px text-[10px] text-forest">已验证</span>
          ) : (
            <span className="rounded-full bg-oxblood/15 px-2 py-px text-[10px] text-oxblood">未验证</span>
          )}
        </span>
      </FieldRow>
      <FieldRow label="角色">{roleText[user.role]}</FieldRow>
      <FieldRow label="计划">{planText[user.plan]}</FieldRow>
      <FieldRow label="账户状态">{user.status === "active" ? "正常" : "已停用"}</FieldRow>
      <FieldRow label="注册时间">{new Date(user.createdAt).toLocaleString("zh-CN")}</FieldRow>
      <FieldRow label="上次登录">{new Date(user.lastLoginAt).toLocaleString("zh-CN")}</FieldRow>
    </div>
  );
}

function SecuritySection({ user }: { user: AccountUser }) {
  const setUser = useAuthStore((s) => s.setUser);
  ...
  return (
    <div className="space-y-5 text-sm">
      <section>
        <h4 className="label-caps mb-2 text-[11px] text-ink-faint">邮箱验证</h4>
        {user.emailVerified ? (
          <p className="text-xs text-forest">✓ {user.email} 已完成验证</p>
        ) : (
          <div className="flex items-center gap-2 text-xs">
            <span className="text-oxblood">{user.email} 尚未验证</span>
            <button
              type="button"
              disabled={verifySent}
              onClick={() => {
                void api.sendVerificationCode(user.email);
                setVerifySent(true);
              }}
              className="rounded border border-hairline px-2 py-0.5 text-ink-soft hover:border-brass disabled:opacity-40"
            >
              {verifySent ? "验证码已发送（演示码 246810）" : "发送验证邮件"}
            </button>
          </div>
        )}
      </section>
      ...
      <section>
        ...
        <div className="max-w-xs space-y-2">
          <input
            ...
            className="w-full rounded-md border border-hairline bg-paper px-2.5 py-1.5 text-sm outline-none focus:border-brass"
          />
          <input
            type="password"
      ...
      <section>
        <h4 className="label-caps mb-2 text-[11px] text-ink-faint">两步验证（2FA）</h4>
        <p className="rounded border border-dashed border-brass/50 px-2.5 py-2 text-xs text-ink-soft">
          未接入 —— 待 auth 契约冻结后提供 TOTP 绑定。
        </p>
      </section>
    </div>
  );
...
function SessionsSection({ user }: { user: AccountUser }) {
  const signOut = useAuthStore((s) => s.signOut);
  return (
    <div className="space-y-4 text-sm">
      <section>
        <h4 className="label-caps mb-2 text-[11px] text-ink-faint">当前会话</h4>
        <div className="flex items-center gap-3 rounded-md border border-hairline bg-paper px-3 py-2">
          <span className="h-2 w-2 rounded-full bg-forest" />
          <div className="text-xs">
            <p className="font-medium text-ink">此浏览器（本机）</p>
            <p className="text-ink-faint">登录于 {new Date(user.lastLoginAt).toLocaleString("zh-CN")}</p>
          </div>
          <span className="ml-auto rounded-full bg-forest/15 px-2 py-px text-[10px] text-forest">当前</span>
        </div>
        <p className="mt-1.5 text-xs text-ink-faint">MOCK：会话列表与远程登出待 auth 契约冻结后接入。</p>
      </section>
      <button
        type="button"
        onClick={() => {
  ...
  return (
    <div>
      <p className="mb-3 rounded-md border border-dashed border-brass/60 bg-brass/5 px-3 py-2 text-xs leading-relaxed text-brass">
        MOCK ONLY —— 成员与权限模型尚未进入 contracts。以下为界面演示位，不可编辑。
      </p>
    ...
    >
      ...
      >
        ...
        <div className="min-w-0 flex-1 overflow-y-auto p-5">
          ...
          {section === "sessions" && <SessionsSection user={user} />}
