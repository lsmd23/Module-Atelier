...
- FE-001: implement M0 web shell and contract-aware workspace boundary — integrated into `main`; launcher/local-account flow is included.
...
- BE-003 (Backend, integrated): storage switch to the local-first architecture in `docs/STORAGE.md` — SQLite with one database per project, app-level database, data directory bootstrap, connection registry, migrations and tests on SQLite.
...
- FE-002 (Integration, next): add Playwright smoke for setup/login → launcher → project/document save → conflict recovery against the local API.
