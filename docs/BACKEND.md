...
| PostgreSQL | 16+ | Local conda instance on port 5433 (`~/.module-atelier/pgctl`) — being replaced by SQLite, see the note at the top |
...
The product is a locally installed application: one process on `127.0.0.1`, the
built frontend served by that same process, and data in a self-contained user
data directory (`docs/STORAGE.md`). There is no server deployment path — the
container definitions that used to live in `infra/` were removed with the
PostgreSQL backing store, because they described deploying a service to a host
instead of running the app on the user's machine.
