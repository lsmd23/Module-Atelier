...
const directiveKinds = new Set(["readaloud", "rule", "development", "treasure", "experience", "pagebreak", "columns", "statblock"]);
...
function parseBlocks(document: ModuleDocumentInput, source: string, diagnostics: Diagnostic[], fullSource = source, sourceOffset = 0): ModuleNode[] {
  ...
  while (index < lines.length) {
    ...
    if (directive?.[1]) {
      ...
      if (!directiveKinds.has(kind)) {
        ...
        nodes.push({ id: nodeId(document.id, "columns", sourceOffset + currentOffset), nodeType: "columns", count: Number.isFinite(count) ? count : 2, children: parseBlocks({ ...document, content: body }, body, diagnostics, fullSource, sourceOffset + currentOffset + line.length + 1), sourceLocation: loc(currentOffset, endOffset) });
      } else {
        ...
        const children = parseBlocks({ ...document, content: body }, body, diagnostics, fullSource, sourceOffset + currentOffset + line.length + 1);
...
export function parsePublishSnapshot(snapshot: PublishSnapshot): ModuleIR {
  ...
  const documents: DocumentNode[] = snapshot.documents.map((document) => ({
    ...
    children: nestSections(document, parseBlocks(document, document.content, diagnostics)),
  }));
