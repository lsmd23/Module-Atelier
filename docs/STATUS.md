...
- BE-003 local-first storage switch is integrated: SQLite app database, per-project database files, data-directory bootstrap, project registry and SQLite test baseline.
...
- BE-002 phase 2 (project ownership, members, role enforcement) is next; phases 3-5 cover the document tree, the asset store and packaging.
- Publishing technical spike (PUB-001) completed locally; shared PublishSnapshot/Module IR contract remains under review.

Blocked:
- No hard blocker. The next integration gate is browser smoke plus project ownership/role enforcement.
...
- v0.4.0: local-account contract replaces email verification; login uses `username`, setup is first-run owner creation, and `AccountUser.email` is nullable.

Integration Risks:
- Storage is now SQLite with one app database and one database per project (`docs/STORAGE.md`). The old PostgreSQL runbook is historical and must not be used for the current local-first runtime.
- Container files remain as unverified packaging experiments; the supported development path is the local Fastify process plus the Vite dev proxy.
...
- No mail transport by design: local accounts use username/password setup and do not expose verification-code flows.
...
- Current merged-main verification: `pnpm install --no-frozen-lockfile`, `pnpm typecheck`, `pnpm test` (131 tests), and `pnpm web:build` pass. No full browser Web↔API E2E has been run yet.
